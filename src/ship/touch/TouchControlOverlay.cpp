...
void TouchControlOverlay::RebuildLayout(ImVec2 displaySize) {
    ...
    const float scale = Context::GetRawInstance()->GetConsoleVariables()->GetFloat("gTouch.Scale", 1.0f);
    ...
    const bool edge = Context::GetRawInstance()->GetConsoleVariables()->GetInteger("gTouch.EdgeLayout", 0) != 0;

    if (!mOcarinaLayout) {
        if (edge) {
            // hug the right edge/corner: thumbs rest at the phone's rim
            ...
            // classic: cluster sits slightly inward (default; preferred in device testing)
            circle(TouchElementId::A, 0.90f * W - sr, 0.72f * H, 0.085f * H, BTN_A, "A");
            circle(TouchElementId::B, 0.80f * W - sr, 0.80f * H, 0.065f * H, BTN_B, "B");
        }
        const float cX = (edge ? 0.918f : 0.865f) * W - sr;
        const float cY = (edge ? 0.42f : 0.47f) * H;
        const float cSp = (edge ? 0.080f : 0.085f) * H * scale;
    ...
    if (!mOcarinaLayout && Context::GetRawInstance()->GetConsoleVariables()->GetInteger("gTouch.ShowDpad", 0)) {
...
void TouchControlOverlay::HandlePillRelease(TouchElementId id) {
    auto gui = Context::GetRawInstance()->GetWindow()->GetGui();
    switch (id) {
    ...
    switch (type) {
        case SDL_FINGERDOWN: {
            ...
            if (el != nullptr) {
                ...
                if (Context::GetRawInstance()->GetConsoleVariables()->GetInteger("gTouch.Haptics", 1)) {
                ...
                const bool fixed =
                    Context::GetRawInstance()->GetConsoleVariables()->GetInteger("gTouch.FixedStick", 0) != 0;
        ...
        case SDL_FINGERMOTION: {
            ...
            if (it->second == TouchElementId::Stick) {
                ...
                const float sens =
                    Context::GetRawInstance()->GetConsoleVariables()->GetFloat("gTouch.CameraSensitivity", 1.0f);
...
void TouchControlOverlay::Draw() {
    ...
    // rebuild when the display changes or when a layout-affecting setting is flipped live
    static float sLastScale = -1.0f;
    static int sLastEdge = -1;
    auto cv = Context::GetRawInstance()->GetConsoleVariables();
    ...
    const float opacity = Context::GetRawInstance()->GetConsoleVariables()->GetFloat("gTouch.Opacity", 0.35f);
    ...
    if (!menuOpen && mStickActive) {
        ...
        const bool edgeHint =
            Context::GetRawInstance()->GetConsoleVariables()->GetInteger("gTouch.EdgeLayout", 0) != 0;
